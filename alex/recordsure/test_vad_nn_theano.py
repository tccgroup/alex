...
crossvalid_frames = int((1.00 * max_frames ))  # cca 20 % of all training data
usec0=0
usedelta=False
useacc=False
mel_banks_only=1
...
def gen_features(speech_data, speech_alignment):
    ...
    crossvalid_x = np.array([frame for frame, _ in vta]).astype(np.float32)
    crossvalid_y = np.array([label for _, label in vta])
...
#    train_x /= tx_std
    ...
    return crossvalid_x, crossvalid_y, tx_m, tx_std
...
def test_nn(speech_data, speech_alignment, fInModelName, pOut):
    print
...
#        train_y = np.load(f)
        ...
        crossvalid_x, crossvalid_y, tx_m, tx_std = gen_features(speech_data, speech_alignment)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="This program test a neural network VAD model.")
...
#    parser.add_argument('--audio-list', help='Audio input list', default=None)
    ...
    parser.add_argument('-m', '--model', help='NN model file', required=True)
...
#    train_speech_alignment.append('model_voip_cs/aligned_best.mlf')
    ...
    test_nn(train_speech, train_speech_alignment, pOut, doFeatureOnly=doFeatureOnly)
